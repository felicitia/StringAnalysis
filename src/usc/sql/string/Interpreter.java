...
import usc.yixue.ResourceAnalysis;
...
public class Interpreter {
	...
	private Set<String> interpret(Variable ir,int itr)
	{
		...
		{
			Set<String> s = new HashSet<>();
			if(((ExternalPara) ir).getType() != null){
				//if the value comes from strings.xml
				if(((ExternalPara) ir).getType().equals("strings.xml")){
					String value = stringRMap.get(((ExternalPara) ir).getName());
					s.add(value);
					
				}
			}else{
				//in this else, the ExternalPara.getType() is null
				if(fieldMap!=null)
				{
					/*
